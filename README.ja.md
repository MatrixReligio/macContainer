...
> **プレリリース：** バージョン0.1.8はAppleシリコンとmacOS 26以降が必要です。重要なコンテナデータは別途バックアップし、破壊的な操作の前にすべての値を確認してください。
...
- 初回使用時に起動検証済みの Alpine 3.22 仮想マシンイメージを自動準備し、作成後は内蔵ターミナルからアクセスできます。
- コンテナガイドでは、OCI イメージ、コンテナネットワーク、名前付き永続ボリュームを1つの確認可能なフローで接続します。
